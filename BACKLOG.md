...
- **Dallas clock (`FD502/distortc.c`):** control registers 0xD/0xE/0xF are read as zero (IRQ/busy/hold, standby, 12/24-mode readback); clock is read-only — register writes other than the format bit are ignored. Still open.
- **SuperIDE (`SuperIDE/idebus.c`):**
  ...
  - ❌ **Still open — most other command opcodes** (0x10-0x1F recalibrate variants) remain no-ops. Lower priority: these aren't part of the normal read/write/identify path real CoCo IDE drivers exercise.
