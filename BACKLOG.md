...
- **FD502 config UI:** "Physical Disks" combo boxes and `FDRAWREAD` text are Windows-only leftovers, always disabled (`fd502.c:407-430`). Left open, grouped with the still-open §5.1 RAW-disk-path decision below — removing this UI is effectively half of that same product decision (keep/port vs. strip the RAW path), not a UI cleanup independent of it.
